...
> ⚠️ **HISTORICAL — round-13 session log. SUPERSEDED.** Rounds 14–17 (the ocean rebuild,
> per-voxel physics, end-game removal) came AFTER this and live in `docs/ROUND14_OCEAN_WORKLOG.md`.
> **For current state read `CLAUDE.md` first — when this file disagrees with the code, the code wins.**
> Lines below that no longer hold: dev port is **5173** (not 5180); the LOST-AT-SEA end-game was later
> **removed** (r17, commit `0095390`); the single band-limited FFT, `SWELL_AMP 0.80`, and the 6 physics
> levers are all superseded. Kept as a dated record of round 13.

Single source of truth for *that* unattended session (round 13). Newest entries at the TOP of the LOG section.
...
- Verify GPU/shader changes IN-BROWSER (Playwright MCP, dev server port 5173), not just tsc/tests.
  Screenshots: save with a RELATIVE filename → lands in the projects ROOT (projects/<name>.png), then Read it.
...
- [x] GAMEPLAY (a) sink ≠ instant game-over (commit 627b022). Enemy sink = non-terminal (salvage+
      ...
      ⚠️ SUPERSEDED (r17, commit 0095390): the LOST-AT-SEA / PRIZE end-game was REMOVED — a sinking
      ship just continues the voyage now (no banner, no game-over). See CLAUDE.md.
