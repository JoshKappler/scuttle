...
> ✅ **SHIPPED & SUPERSEDED.** This phase-1 ocean shipped, then was rebuilt by the "Ocean
> Rebuild" (rounds 14–17, see `2026-06-13-ocean-rebuild-design.md` + `docs/ROUND14_OCEAN_WORKLOG.md`):
> the single band-limited FFT here became a 3-cascade Tessendorf ocean. **Its one permanent legacy is
> the rule that physics rides ONLY the analytic ≥14 m Gerstner swell** (`sim/gerstner.ts`,
> `PHYSICS_MIN_WAVELENGTH`). For current state read `CLAUDE.md`; the code is the source of truth, not this spec.

**Date:** 2026-06-12
**Status:** Shipped (phase 1); superseded by Ocean Rebuild (r14–17)
