...
async function main() {
    ...
    if (controls.lmbPressed) {
      ...
      if (controls.aiming) {
        if (plugChannel <= 0) {
          ...
          cannons.fireBroadside(sloop, aimUI.aimBearing(), t, () => ({
  ...
  const fitViewport = () => {
    ...
    aimUI.setResolution(w, h); // fat aim lines size their width in px
  };
  ...
  function updateHud(dt: number, tr: { x: number; y: number; z: number }): void {
    ...
    hudEls.gunBar.style.width = `${aimUI.gunReadout().frac * 100}%`;
    ...
    // Flood strip is DISPLAY-only — the sim's per-compartment waterVolume is faithful, but a breached
    // hull that's still floating equalises at only ~20-40% of TOTAL air capacity, so frac (water/air)
    // never gets near 1 and every "visibly flooding" hold read as a tiny sliver. Normalise the BAR
    // against FLOOD_FULL_REF (a hold half-full of its own air is already a serious flood → reads high)
    // and give any wet hold a minimum visible stub so a seeped compartment shows clearly. The true frac
    // still drives the dry/full COLOUR classes, so "full" red still means genuinely brim-full.
    const FLOOD_FULL_REF = 0.5;
    sloop.build.compartments.forEach((c, i) => {
      const fill = floodFills[i];
      if (!fill) return;
      const frac = c.volume > 0 ? c.waterVolume / c.volume : 0;
      const display = Math.min(1, frac / FLOOD_FULL_REF);
      // any wet hold (incl. ones filled purely by SEEP) shows at least a 6% stub; bone-dry shows nothing.
      const heightPct = frac > 0.005 ? Math.max(display * 100, 6) : 0;
      fill.style.height = `${heightPct}%`;
      const cls = frac < 0.005 ? "dry" : frac > 0.97 ? "full" : "flooding";
      if (cls !== floodCls[i]) {
  ...
  const devPanel = createDevPanel([
    ...
    },
    {
      ...
      controls: [
        ...
        { type: "slider", label: "hull shade floor", obj: TUN.gfx.hull, key: "shadeFloor", min: 0, max: 3, step: 0.05 },
  ...
  renderer.setAnimationLoop(() => {
    ...
    const bearNow = aimUI.aimBearing();
    ...
    sloopVisual.animate(
      ...
      sailing.sailSet,
      controls.aiming
        ? { bearing: aimUI.aimBearing(), elevationDeg: controls.elevationDeg, traverseDeg: controls.traverseDeg }
        : null,
