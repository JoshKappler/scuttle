...
- **Voxel destruction (one rule)** — cannonballs bore clean holes; on a ram the two hulls (kept OUT of Rapier's rigid solver) interpenetrate and `voxelContact` resolves it in three coupled parts: **carve** the real overlapping voxels of BOTH hulls on a closing-energy budget, **cancel** the closing to a common velocity at COM height (off-centre → yaw/PIT, never roll), and **de-penetrate by POSITION** so two solid hulls can never phase through. The keel's own anisotropic water drag bleeds the struck hull's lurch (no "rammer/target" case); breached compartments flood by Bernoulli and she founders.
...
- `crush`: ship-vs-ship deformable contact (`enabled, vBreak 2, maxStepEnergy 5e6, yield 1, fling 1, minDepth 0.06, maxDvPerStep 4, depen 0.5`). Ship-ship is OUT of Rapier's solver (physics.ts), so the hulls interpenetrate and `voxelContact` reads the real `voxelOverlap` each step and resolves it in three coupled parts, NO "rammer/target": **(1) carve** — where the hulls' solid voxels overlap and close ≥ `vBreak`, break the cheapest of those EXACT overlapping cells of BOTH hulls on a budget of ½·μ·vClose² (cap `maxStepEnergy`); **(2) cancel** — an inelastic impulse along the overlap push-out axis drives them to a common velocity (cap `maxDvPerStep`/step), applied at COM height → yaw (PIT) never roll, then the keel's own ~42×-sideways water drag (ship.ts) bleeds the struck hull's lurch (the "molasses"); **(3) de-penetrate** — shove the bodies apart in POSITION by the overlap depth × `depen` (inverse-mass split, re-solved each step so it can't accumulate/fling). Part 3 is the hard non-penetration that stops a hull phasing through; carving just sets the wood cost. Replaces the retired `ram` levers AND the brief Rapier-rigid-hull experiment (it starved the carve — hollow hulls barely overlap under a rigid solver). Wood softened: `sim/materials.ts STRENGTH_TO_JOULES = 5000` (oak 15 kJ/cell).
