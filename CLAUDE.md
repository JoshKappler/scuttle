...
- **Voxel destruction (one rule, Teardown-style)** — cannonballs bore clean holes; on a ram the two hulls (kept OUT of Rapier's rigid solver) interpenetrate and `voxelContact` branches PER voxel-contact on the closing speed: **above `vBreak`** both voxels are destroyed and the fracture energy is taken out of the closing motion (`crush.breakImpulse`), so the hull keeps most of its speed and PLOWS into the cleared space next step — non-penetration is free because the voxel in the way is gone (no "jar"); **below `vBreak`** it rests — cancel the closing and de-penetrate by POSITION (the ONLY place that runs). The bite is horizontal at COM height (off-centre → yaw, never roll) and split by mass (heavier = harder to shove); the keel's water drag bleeds the struck hull's lurch; breached compartments flood and she founders. Everything on the ship is real grid voxels (hull, deck, quarterdeck, cabin, bulwark, ballast, bow armor); cannons/wheel/masts/sails are separate meshes, never in the grid, so the carve can't touch them.
...
- `crush`: ship-vs-ship deformable contact — the Teardown rule (`enabled, vBreak 2, toughness 1, buffer 0.4, depen 0.3, maxDepenSpeed 3, biteDvCap 6, maxStepEnergy 5e6, minDepth 0.04, fling 1`). Ship-ship is OUT of Rapier's solver (physics.ts) so the hulls interpenetrate; `voxelContact` reads the real per-voxel overlap (`sim/voxelOverlap.detectContacts`, allocation-light scratch + a `buffer`-voxel "close enough" tolerance) each step and branches PER CONTACT on the HORIZONTAL closing speed (so wave heave never reads as closing, and the bite at COM height yaws never rolls): **closing > `vBreak`** → BREAK both voxels; the fracture energy (`Σ breakEnergy × toughness`) is removed from the closing motion as an inelastic bite (`crush.breakImpulse`, cap `biteDvCap`, split by mass → heavier barely slows and guts a light hull). Only the thin overlapping layer breaks per step, so she keeps speed and plows into the cleared space next step — non-penetration is FREE here (nothing left to push), no "jar". **closing ≤ `vBreak`** → REST: cancel the closing + de-penetrate by POSITION (`depth × depen`, rate-capped `maxDepenSpeed`); this is the ONLY place positional separation runs (the old design ran it every step EVEN while breaking — that WAS the jar). `maxStepEnergy` is just an anti-vaporize clamp for a teleport-deep overlap. **Perf:** the carve's downstream `flushDamage` heavy recompute stays ~10 Hz, but the ~40 ms `rebuildDeckCollider` (walkable-deck trimesh) is now DEBOUNCED (rebuilds only once carving pauses, `ship.ts` COLLIDER_QUIET/MAX_STALE) — it was firing every 6 steps mid-ram, the impact-lag cause. Replaces the retired `ram` levers, the 3-part carve/cancel/de-penetrate-every-step rule, AND the Rapier-rigid-hull experiment (it starved the carve — hollow hulls barely overlap under a rigid solver). Wood: `sim/materials.ts STRENGTH_TO_JOULES = 5000` (oak 15 kJ/cell).
