...
- **Voxel destruction (one rule)** — cannonballs bore clean holes; hulls deform on contact via a symmetric *inelastic* crunch (voxels break + momentum trades toward a common velocity; the keel's own anisotropic water drag holds the struck hull so it lurches but never flies — no "rammer/target" special case); breached compartments flood by Bernoulli and she founders.
...
- `crush`: ship-vs-ship **inelastic-collision** contact (`enabled, vBreak 2, maxStepEnergy, yield, transfer 0.5, separate 0.6, fling, minDepth, maxDvPerStep 4`). ONE symmetric rule, NO "rammer/target": where the hulls overlap and close ≥ `vBreak`, the cheapest contacting voxels of both break on a budget of ½·μ·vClose², and an inelastic impulse along the **relative-velocity direction** drives both toward their common velocity (faster slows, slower speeds up) until the relative motion — and the breaking — stop. `transfer` is the inelastic fraction (1 = full common velocity / most epic; lower = "not pushed as easily"). The struck ship is NOT rooted — the keel's own anisotropic water drag (ship.ts, ~42× stronger sideways than fore/aft) bleeds the velocity it gains → the "molasses" feel, emergent. Self-limiting (vRel→0 ⇒ impulse→0) and uses the velocity direction not a centre-to-centre normal (which flipped/spiked at deep overlap), so it can't fling; applied at COM height → yaw (PIT) never roll; `maxDvPerStep` is a per-step smoothing/NaN cap. Replaces the retired `ram` levers. Wood softened: `sim/materials.ts STRENGTH_TO_JOULES = 5000` (oak 15 kJ/cell).
